...
app.config['SECRET_KEY'] = os.getenv('JWT_SECRET_KEY', 'dev-secret-key-12345')
CORS(app)
...
def log_action(action):
    """تسجيل أي إجراء"""
    temp_data['logs'].append(f"[{datetime.now().strftime('%H:%M:%S')}] {action}")
...
def whatsapp_page():
    ...
    return render_template('whatsapp.html', 
                          qr_path=url_for('static', filename='qr_code.png'))
...
def api_whatsapp_qr():
    ...
    try:
        ...
        if result['status'] == 'qr_ready':
            ...
            log_action("تم إنشاء QR Code للواتساب")
...
class LastBotCore:
    ...
    def __init__(self):
        ...
        logging.info("✅ تم تهيئة LastBotCore")
...
class AuthManager:
    ...
    def create_default_admin(self):
        ...
        default_user = {
            "id": "admin_001",
            "username": "admin",
            "password_hash": "admin123",  # في الحقيقة يجب استخدام bcrypt
            "role": "admin"
        }
        
        with open(self.users_file, 'w') as f:
            json.dump({"users": [default_user]}, f, indent=2)
        logging.info("✅ تم إنشاء مستخدم admin افتراضي")
...
if __name__ == '__main__':
    ...
    # تشغيل Flask
    logging.info("🌐 جاري تشغيل خادم Flask على http://localhost:5000")
    app.run(
        host='0.0.0.0',
